...
class UdpP2PClient:
    ...
    def _punch_hole(self, target_ip, target_port):
        ...
        for _ in range(5):
            try:
                logging.info(f"sending: HOLE_PUNCH to {(target_ip, target_port)}")
    ...
    def _send_heartbeat(self):
        ...
        while self.running:
            ...
            try:
                logging.info(f"sending: REGISTER to {(SERVER_PUBLIC_DNS, SERVER_PUBLIC_PORT)}")
    ...
    def _listen(self):
        ...
        while self.running:
            try:
                ...
                data, addr = self.sock.recvfrom(4096)
                logging.info(f"receiving from: {addr} <- {data} ")
