...
class P2PClient:
    ...
    def start(self):
        ...
        while True:
            ...
            # Loop through every Target ID you typed
            for target_id in ids_list:
                found = False  # Track if we found them
                for p in self.known_peers:
                    if str(p['id']) == target_id:
                        self.send_message(msg, p['ip'], p['port'])
                        found = True

                # If we finished the loop and didn't find the ID:
                if not found:
                    print(f"[-] Warning: Message not sent. Peer {target_id} not found.")
